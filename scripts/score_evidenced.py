...
from packages.llmkit.budget import Budget                                   # noqa: E402
...
for j in joint["jointly_covered"]:
    a_id = next(q for q, n in names.items() if n == j["a"])
    b_id = next(q for q, n in names.items() if n == j["b"])
